...
export const createStripe = action({
  ...
  handler: async (ctx, args) => {
    ...
    if (user === null) {
      throw new Error("User not found in the current context");
    }
    ...
    console.log("Current Stripe Account ID:", accountId);

    if (!accountId) {
      const account = await stripe.accounts.create({
        type: "standard", // Uncommented to specify account type
      });
      console.log("New Stripe Account Created:", account);
