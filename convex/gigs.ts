  ...
  handler: async (ctx, args) => {
    ...
    if (args.filter !== undefined) {
      ...
      const filteredGigs = gigs.filter(
        (gig) => gig.subcategoryId === subcategory?._id
      );
      gigs = filteredGigs;
    }
    ...
    if (identity !== null) {
      gigsWithFavoriteRelation = await Promise.all(
        gigs.map((gig) => {
          return ctx.db
            .query("userFavorites")
            .withIndex("by_user_gig", (q) =>
              q.eq("userId", gig.sellerId).eq("gigId", gig._id)
            )
            .unique()
            .then((favorite) => {
    ...
    const gigsWithImages = await Promise.all(
      gigsWithFavoriteRelation.map(async (gig) => {
...
export const getGigsWithOrderAmountAndRevenue = query({
  handler: async (ctx) => {
    ...
    // get images
    const gigsFull = await Promise.all(
      gigsWithOrderAmountAndRevenue.map(async (gig) => {
        const image = await ctx.db
          .query("gigMedia")
          .withIndex("by_gigId", (q) => q.eq("gigId", gig._id))
          .first();
